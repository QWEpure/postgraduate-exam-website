...
const SUBJECT_LABELS = { ds: '数据结构', co: '计算机组成原理', os: '操作系统', cn: '计算机网络' }
...
function buildIndexRow(question, year) {
  ...
  return {
    ...
    type: question.questionType === 'comprehensive' ? 'comprehensive' : 'choice',
    subject: question.subject,
    chapter: question.chapterName,
    topic: SUBJECT_LABELS[question.subject] || question.subject,
    stemText: question.stem ?? '',
    knowledgeBlockIds: Array.isArray(question.knowledgeBlockIds) ? question.knowledgeBlockIds : [],
...
function main() {
  ...
  if (!years.length) {
    console.error(`未在 ${path.relative(projectRoot, examsRoot)} 找到任何年份目录。`)
    process.exit(1)
  }
  ...
  for (const year of years) {
    ...
    if (!existsSync(paperPath)) {
      console.error(`缺少 ${path.relative(projectRoot, paperPath)}，已跳过该年份。`)
      process.exitCode = 1
      continue
    }
  ...
  indexRows.sort((left, right) => left.year - right.year || left.number - right.number)

  writeFileSync(
    manifestPath,
    JSON.stringify({ version: MANIFEST_VERSION, totalQuestions, years: manifestYears }, null, 2) + '\n',
  )
  ...
  console.log(`产物：${path.relative(projectRoot, manifestPath)}、${path.relative(projectRoot, indexPath)}`)
}
