 ...
 */
export function parseExactYearNumber(input: string): { year: number; number: number } | null {
  const text = input.trim()
  if (!text) return null
  const fullYear = text.match(/(20\d{2})\s*[年.·\-_/，,。\s]*\s*(\d{1,2})\s*[题小问]?/)
  if (fullYear) return { year: Number(fullYear[1]), number: Number(fullYear[2]) }
  const shortYear = text.match(/(\d{2})\s*年\s*第?\s*(\d{1,2})\s*[题小问]?/)
  if (shortYear) return { year: 2000 + Number(shortYear[1]), number: Number(shortYear[2]) }
  return null
}
 ...
 */
export function searchExam(query: string, loaded: LoadedCorpus): SearchResult[] {
