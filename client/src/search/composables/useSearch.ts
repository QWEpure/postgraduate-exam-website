 ...
 *
 * 数据来源：构建期预生成的 /search/search-index.json（倒排索引 + 子点文档 + 章节签名）
 *           + /search/synonyms.json（同义词组）。
 * 检索策略：知识走倒排索引 + 字段分层（标题层优先于正文）+ 同义词扩展 + segmentit 分词；
 ...
 * 全站搜索 composable。
 */
export function useSearch(options?: { debounceMs?: number; topK?: number }): UseSearchReturn {
  ...
  async function search(overrideQuery?: string): Promise<SearchOutcome | null> {
    const q = (overrideQuery != null ? overrideQuery : query.value || '').trim()
    ...
    return new Promise<SearchOutcome | null>((resolve) => {
      debounceTimer = setTimeout(() => { runSearch(q).then(resolve) }, debounceMs)
    })
  }
