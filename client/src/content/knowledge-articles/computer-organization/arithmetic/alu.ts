    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          id: 'kb-co-alu-2-2',
          type: 'paragraph',
          text: '| 标志 | 名称 | 何时置 1 |\n|---|---|---|\n| ZF | 零标志 | 结果每一位都为 0 |\n| SF | 符号标志 | 结果最高位（符号位）为 1，即结果为负 |\n| CF | 进位/借位标志 | 加法时最高位产生进位；减法时最高位产生借位 |\n| OF | 溢出标志 | 符号位进位 Cin 与最高位进位 Cout 不相同 |',
        },
        {
          id: 'kb-co-alu-2-4',
          type: 'paragraph',
          text: String.raw`减法 $A - B$ 在硬件里变成 $A + \bar{B} + 1$（B 按位取反，最低位进位 1）。加法器最高位的进位输出 $C_{out}$ 够减时是 1、不够减时是 0。

CF 是**借位标志**，硬件把 $C_{out}$ **取反**后存入：

- 够减（$A \ge B$）：$C_{out}=1$，CF $= \overline{C_{out}} = 0$，表示没有借位。
- 不够减（$A < B$）：$C_{out}=0$，CF $= \overline{C_{out}} = 1$，表示发生了借位。

不取反时，"够减时 CF=1、不够减时 CF=0"，与"CF=1 表示有借位"相反。取反后 CF=1 表示不够减，正好作为借位标志。

加法时 CF=$C_{out}$（有进位），减法时 CF=$\overline{C_{out}}$（有借位），这是"进位/借位标志"名称的由来。无符号比较大小：减法后 CF=1 说明 $A < B$。`,
        },
        {
          id: 'kb-co-alu-2-3',
          type: 'callout',
          title: '计算机眼里 OF 和 CF 是怎么算的',
          text: String.raw`计算机不做"有没有溢出"的语义判断，只看最高位的两个进位：最高位的进位输出 $C_{out}$ 和进入符号位的进位 $C_{in}$。加法时 CF 取 $C_{out}$（有进位），减法时 CF 取 $\overline{C_{out}}$（有借位）；OF 是 $C_{out}$ 与 $C_{in}$ 的异或，$C_{out} \oplus C_{in} = 1$ 表示符号位被破坏，即溢出。`,
          tone: 'orange',
        },
      ],
    },
